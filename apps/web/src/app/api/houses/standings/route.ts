...
export async function GET() {
  ...
  const [recognitions, houseEvents] = await Promise.all([
    context.admin.from('recognition_logs').select('house_snapshot, point_value, r_values(name), domains(name)').limit(10000),
    context.admin.from('house_events').select('house, point_value').limit(10000),
  ])
