...
for (const seed of recognitionSeed) {
  ...
  const { error } = await admin.from('recognition_logs').insert({
    ...
    r_value_snapshot: (rValues ?? []).find((row) => row.key === seed.r)?.name ?? seed.r,
    behaviour_label_snapshot: seed.note,
